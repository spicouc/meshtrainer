...
for i in 1 2 3 4 5 6; do
    ...
    cp "$TMPDIR"/*.py "$WORKDIR"/ 2>/dev/null || true; cd "$WORKDIR"
    case $i in
        ...
        2) NAME="MUT-P1-02-ignore-labels"
           sed -i 's/loss = loss_fn(shift_logits.view(-1, P.vocab_size), shift_labels.view(-1))/loss = loss_fn(shift_logits.view(-1, P.vocab_size), torch.zeros_like(shift_labels))/' rc5_2_numerical_models.py ;;
        3) NAME="MUT-P1-03-bypass-worker"
           sed -i 's/for layer in self.local_layers:\n            x = layer(x, src_mask=causal_mask)/x = x  # bypass worker/' rc5_2_numerical_models.py ;;
    ...
    set +e; timeout 300 python3 rc5_2_phase1_tests.py > "mutation_$NAME.log" 2>&1; S=$?; set -e
    [ $S -eq 124 ] && echo "  $NAME: TIMEOUT" && RESULTS+="$NAME: TIMEOUT"$'\n' && TIMEOUT_COUNT=$((TIMEOUT_COUNT+1)) && continue
    [ $S -ne 0 ] && echo "  $NAME: DETECTED" && RESULTS+="$NAME: DETECTED"$'\n' && SCORE=$((SCORE+1)) && continue
    echo "  $NAME: NOT DETECTED" && RESULTS+="$NAME: NOT DETECTED"$'\n'
done
echo ""; echo "============================================"
echo "$RESULTS"; echo "Mutation score: $SCORE/$TOTAL"
echo "Invalid: $INVALID, Not applied: $NOT_APPLIED, Timeouts: $TIMEOUT_COUNT"
echo "============================================"
